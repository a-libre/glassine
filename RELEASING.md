...
Before it: the changelog (`docs/site/about/changelog.mdx`) and the top block of `site/whats-new.md` — three to five lines for the version, which the landing page shows under *What's new*. `release.sh` runs `site/update.sh`, which puts those lines and the version into `site/index.html`, so the page is never behind the download it offers; `site/update.sh --check` says whether it is. The pictures the landing page, the README and the manual share come two ways. Six are taken by hand in *Glassine Demo* (`./build.sh --demo`) with the backdrops chosen by eye — the App Store set, converted by `docs/appstore/from-captures.py` — and `site/from-store-shots.sh` brings them in at 1440 × 900; the landing page uses only these. The other five (focus, the formatting bar, the slash menu, the light theme, sync connected) the app takes of itself with `site/screenshots.sh`, each over a backdrop, for the README and the manual. When the app's look changed, retake what changed in the same commit as the change, so the pictures never show an older app than the words describe.
