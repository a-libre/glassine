...
Before it: the changelog (`docs/site/about/changelog.mdx`) and the top block of `site/whats-new.md` — three to five lines for the version, which the landing page shows under *What's new*. `release.sh` runs `site/update.sh`, which puts those lines and the version into `site/index.html`, so the page is never behind the download it offers; `site/update.sh --check` says whether it is. When the app's look changed, `site/screenshots.sh` retakes the eleven pictures the landing page, the README and the manual share (from the direct build, in the showcase library, about two minutes with the window in front) — do that in the same commit as the change, so the pictures never show an older app than the words describe.
