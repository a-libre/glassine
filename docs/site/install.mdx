...
[**Glassine Writer** on the Mac App Store](https://apps.apple.com/us/app/glassine-writer/id6808032066?mt=12) — free; the store updates it. The store has version 0.2.3; what came after it in the direct download (the settings, sync with GitHub, the rule fix) follows in the next store build.
...
macOS asks whether Glassine may reach files in iCloud Drive; the prompt says what for — the Glassine folder, read to list your documents and written to as you type. Say yes; that is where the library lives. A **Glassine** folder appears at the top level of iCloud Drive with a welcome document inside it, and you are in the editor.
