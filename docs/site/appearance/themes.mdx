...
A theme is the glass — the blur material, the tint over it and how strongly the tint covers the blur, an optional paper grain, the sidebar's own tint — and the colours: text, headings, accent, Markdown syntax, quotes, code, links, caret and selection. The whole window follows it, Review's Glass style included. What the glass looks through — the desktop, or a [backdrop](/appearance/backdrop) of the app's own — is a separate setting.
...
Settings → Theme → Appearance. *Always the chosen theme* keeps whatever you picked. *Follow the system* takes one light theme and one dark theme — Paper and Dusk to begin with — and switches between them when macOS does. Choosing a theme from the View menu while following the system fills the matching slot.
...
<Steps>
  <Step title="Duplicate">
    Settings → Theme. Click the tile of the built-in theme closest to what you want and press **Duplicate**. A copy appears under *Mine* and becomes the current theme.
  </Step>
...
Themes export as small JSON files, `Name.glassinetheme.json`, with **Export…** in Settings → Theme; **Import…** beside it brings one in. They are easy to keep in a dotfiles repo, send to someone, or post; two of the built-ins, Dusk and Lavender, began life as exported files.
