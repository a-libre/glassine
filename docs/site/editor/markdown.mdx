...
The file is plain Markdown. The editor shows it styled — headings larger, emphasis in weight, code in a box — but leaves every mark in place, dimmed to the theme's syntax colour, so what you see is what is in the file. There is no rendered mode to fall out of and no hidden formatting; Review, when you want the finished page, is [one key away](/review/review). If you would rather not see the marks at all, [hide them](/editor/formatting#hiding-the-markdown): the file stays the same and the paragraph you are editing still shows its syntax.
