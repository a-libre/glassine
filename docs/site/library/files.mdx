...
Settings → Library → **Change…** points Glassine at any folder — a Dropbox folder, an Obsidian vault, a plain folder on the Mac. **Use iCloud Drive** puts it back. The App Store version remembers a chosen folder with a security-scoped bookmark, so it can return to it after a relaunch without asking again.
...
Glassine opens Markdown and plain-text files that are not in the library too: double-click one in Finder (or right-click → Open With → Glassine), drop it on the Dock icon, or File → **Open…** (⌘O). The file opens *in place* — edited and saved where it is, never copied into the library — and shows in the sidebar under **Elsewhere** for as long as it is open, with the folder it lives in. It is not renamed after its first line, not listed in All Documents, Recents or the Timelapse, and not remembered for the next launch. File → **Add to Library** (also in the row's right-click menu) makes a copy at the top level of the library and opens that instead; the original stays where it was. Rename, Duplicate, Shelve and Move to Trash are for library documents and stay grey while a file from elsewhere is open; Reveal in Finder works as usual.

To make Glassine the app that opens every `.md` file: select one in Finder, ⌘I (Get Info), choose Glassine under *Open with*, and click **Change All…**. The same works for `.txt`.
...
Settings, the list of starred documents, which folders are expanded, the recent list, the sort order and the window's size and position live in the app's preferences, not in the library. Themes you make are stored there too, and can be exported as JSON from Settings → Theme.
