...
<AccordionGroup>
  ...
  <Accordion title="Can I open a file that isn't in the library?">
    Yes. Double-click it in Finder (or Open With → Glassine), drop it on the Dock icon, or File → Open… (⌘O). It opens in place, edited and saved where it is, and shows under *Elsewhere* in the sidebar while it is open; File → Add to Library copies it in. To make Glassine the default for every `.md` file: Get Info on one in Finder, choose Glassine under *Open with*, Change All…. The [files page](/library/files#files-from-elsewhere) has the details.
  </Accordion>

  <Accordion title="Can I use it on a work Mac without signing into iCloud?">
    Yes. Settings → Library → Sync keeps the library in a private GitHub repository of yours; any Mac with Glassine and a token for that one repository has the same documents, and nothing else about your accounts touches it. Every version of every note is kept in the bargain. See [sync with GitHub](/library/sync) — and check what your employer's policy says about personal files on the machine first.
  </Accordion>

  <Accordion title="Can I use Obsidian, iA Writer or Dropbox with it?">
    Yes. Point Settings → Library at any folder — an Obsidian vault, a Dropbox folder — and Glassine reads and writes the same plain Markdown as everything else. Two apps can have the folder open at once; if both change the same file, Glassine keeps your edits and saves the other version beside the file as `… (conflict).md`.
  </Accordion>
  ...
  <Accordion title="Can I have two windows, or two libraries?">
    One window, one library, by design. Switch libraries under Settings → Library.
  </Accordion>

  <Accordion title="I closed the window. How do I get it back?">
    Glassine keeps running with its window closed. Window → Glassine brings it back, so does clicking the icon in the Dock, and so does any command that needs the window — ⌘N, ⌘1, ⌘2, ⌘F, ⌘, — from the menu bar.
  </Accordion>
