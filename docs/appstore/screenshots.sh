...
# a fresh launch with the settings it needs written beforehand; only Review and
# the Daily view need a keystroke. Output: dist/screenshots/*.png at 2880×1800,
# which is one of the four sizes App Store Connect accepts for macOS.
#
# Run this from Terminal, not from a script runner: macOS grants screen capture
# and keystrokes per app, and Terminal will ask for both the first time.
#   Screen & System Audio Recording  — for screencapture
#   Accessibility                    — for the two menu keystrokes
# Allow both, then run it again. Without capture the script cannot work at all;
# without Accessibility it takes the shots that need no keystroke and says which
# ones it skipped.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
# --- Screen geometry ------------------------------------------------------------------------
# Pixels per point on the main display: the panel's resolution over the desktop's
# size in points. On a scaled ("More Space") display this is not a whole number, so
# the window is asked for a hair over the target and the shot is cropped after.
read -r px_w px_h < <(system_profiler SPDisplaysDataType 2>/dev/null \
  | awk -F'[ x]+' '/Resolution:/ {print $3, $4; exit}')
read -r sc_x sc_y sc_w sc_h < <("$HELPER" screen)
[[ -n "${px_w:-}" && -n "${sc_w:-}" ]] || { echo "Could not read the display geometry." >&2; exit 1; }
ratio=$(awk -v a="$px_w" -v b="$sc_w" 'BEGIN {printf "%.6f", a / b}')
win_w=$(awk -v t="$TARGET_W" -v r="$ratio" 'BEGIN {printf "%d", int(t / r) + 2}')
win_h=$(awk -v t="$TARGET_H" -v r="$ratio" 'BEGIN {printf "%d", int(t / r) + 2}')
if (( win_w > sc_w || win_h > sc_h )); then
  echo "The usable screen (${sc_w}x${sc_h} points) is too small for a ${win_w}x${win_h} window." >&2
  echo "Set the display to a larger 'More Space' scale, or lower TARGET_W/TARGET_H to 2560x1600." >&2
  exit 1
fi
...
echo "* Display ${px_w}x${px_h} px over ${sc_w}x${sc_h} pt usable (${ratio} px/pt); window ${win_w}x${win_h} pt"
...
mkdir -p "$OUT"
SKIPPED=()   # shots that needed a keystroke macOS would not deliver
...
shot() {
  ...
  read -r got_w got_h < <(sips -g pixelWidth -g pixelHeight "$OUT/$name.png" | awk '/pixel/ {printf "%s ", $2}')
  if (( got_w < TARGET_W || got_h < TARGET_H )); then
    echo "$name captured at ${got_w}x${got_h}, smaller than ${TARGET_W}x${TARGET_H}." >&2
    echo "The display is scaled differently than system_profiler reported." >&2
    exit 1
  fi
  sips -c "$TARGET_H" "$TARGET_W" "$OUT/$name.png" --out "$OUT/$name.png" >/dev/null
...
shot 3-review   '{'"$base"',"themeID":"ocean","lastOpenedDocument":"Essays/On Writing Slowly.md","caretPositions":{"Essays/On Writing Slowly.md":0}}' 'key code 36 using command down'
shot 4-library  '{'"$base"',"themeID":"ocean","lastOpenedDocument":null}'
shot 5-daily    '{'"$base"',"themeID":"ocean","lastOpenedDocument":"Essays/On Writing Slowly.md"}' 'keystroke "d" using command down'
shot 6-light    '{'"$base"',"themeID":"paper","lastOpenedDocument":"Notes/Launch Checklist.md","caretPositions":{"Notes/Launch Checklist.md":0}}'

if (( ${#SKIPPED[@]} )); then
  echo "* Done, without ${SKIPPED[*]} — those need keystrokes." >&2
  echo "  Turn Terminal on in System Settings > Privacy & Security > Accessibility," >&2
  echo "  then run this again to add them." >&2
else
